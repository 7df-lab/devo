...
3. `run_agent` 解析当前工作目录和 `DEVO_HOME`，先加载工作区 effective app config，再用其中的模型元数据覆盖构建 model catalog，之后解析 permission preset、provider settings 和 saved model entries。
