...
3. `run_agent` resolves the current working directory and `DEVO_HOME`, loads the
   workspace-effective app config, then constructs the model catalog from its
   model metadata overrides before resolving the permission preset, provider
   settings, and saved model entries.
