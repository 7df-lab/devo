#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $DEVO_HOME/skills."""
...
def _devo_home() -> str:
    return os.environ.get("DEVO_HOME", os.path.expanduser("~/.devo"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "devo")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "devo-skill-install")
...
def _default_dest() -> str:
    return os.path.join(_devo_home(), "skills")
