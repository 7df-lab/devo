...
/// Note this thread uses advisory file locking and performs blocking I/O, so it
/// should be used with `tokio::task::spawn_blocking` when called from an async
/// context.
...
/// Note this function uses advisory file locking and performs blocking I/O, so
/// it should be used with `tokio::task::spawn_blocking` when called from an
/// async context.
