...
- From the Devo CLI, run `devo execpolicy check` with one or more policy files (for example `src/default.rules`) to check a command:

```bash
devo execpolicy check --rules path/to/policy.rules git status
```

- To opt into basename fallback for absolute program paths, pass `--resolve-host-executables`:

```bash
devo execpolicy check \
...
```bash
cargo run -p devo-execpolicy -- check --rules path/to/policy.rules git status
```
