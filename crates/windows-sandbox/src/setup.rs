...
pub const OFFLINE_USERNAME: &str = "DevoSandboxOffline";
pub const ONLINE_USERNAME: &str = "DevoSandboxOnline";
const ERROR_CANCELLED: u32 = 1223;
const SECURITY_BUILTIN_DOMAIN_RID: u32 = 0x0000_0020;
const DOMAIN_ALIAS_RID_ADMINS: u32 = 0x0000_0220;
const SETUP_EXE_FILENAME: &str = "devo-windows-sandbox-setup.exe";
...
pub fn sandbox_dir(devo_home: &Path) -> PathBuf {
    devo_home.join(".sandbox")
}

pub fn sandbox_bin_dir(devo_home: &Path) -> PathBuf {
    devo_home.join(".sandbox-bin")
}

pub fn sandbox_secrets_dir(devo_home: &Path) -> PathBuf {
    devo_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(devo_home: &Path) -> PathBuf {
    sandbox_dir(devo_home).join("setup_marker.json")
}

pub fn sandbox_users_path(devo_home: &Path) -> PathBuf {
    sandbox_secrets_dir(devo_home).join("sandbox_users.json")
}
...
fn run_setup_refresh_inner(
    ...
    let payload = ElevationPayload {
        ...
        devo_home: request.devo_home.to_path_buf(),
    ...
    run_setup_singleflight(b64.clone(), || {
        run_setup_refresh_payload(&b64, request.devo_home)
    })
}

fn run_setup_refresh_payload(b64: &str, devo_home: &Path) -> Result<()> {
    let exe = find_setup_exe();
    let sbx_dir = sandbox_dir(devo_home);
    let log_path = current_log_file_path(&sbx_dir);
    let cleared_report = match clear_setup_error_report(devo_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| devo_home.to_path_buf());
    log_note(
        &format!(
...
fn gather_helper_read_roots(devo_home: &Path) -> Vec<PathBuf> {
    let helper_dir = helper_bin_dir(devo_home);
...
fn gather_full_read_roots_for_permissions(
    ...
    let mut roots = gather_helper_read_roots(devo_home);
    roots.extend(
...
pub(crate) fn effective_write_roots_for_permissions(
    ...
    filter_sensitive_write_roots(write_roots, devo_home)
}
...
struct ElevationPayload {
    ...
    otel: Option<crate::otel_stub::StatsigMetricsSettings>,
    real_user: String,
    mode: SetupMode,
    #[serde(default)]
    refresh_only: bool,
...
const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "DEVO_NETWORK_ALLOW_LOCAL_BINDING";
...
fn report_helper_failure(
    ...
    match read_setup_error_report(devo_home) {
...
fn verify_setup_completed(devo_home: &Path) -> Result<()> {
    if sandbox_setup_is_complete(devo_home) {
        Ok(())
    } else {
        Err(failure(
...
fn run_setup_exe(
    ...
    run_setup_singleflight(payload_b64.clone(), || {
        run_setup_exe_payload(&payload_b64, needs_elevation, devo_home)
    })
}

fn run_setup_exe_payload(payload_b64: &str, needs_elevation: bool, devo_home: &Path) -> Result<()> {
...
fn run_elevated_setup_inner(
    ...
    let sbx_dir = sandbox_dir(request.devo_home);
    ...
    let payload = ElevationPayload {
        ...
        allow_local_binding: offline_proxy_settings.allow_local_binding,
        real_user: std::env::var("USERNAME").unwrap_or_else(|_| "Administrators".to_string()),
        otel: crate::otel_stub::global_statsig_metrics_settings(),
        mode: SetupMode::Full,
        refresh_only: false,
    };
    ...
    run_setup_exe(&payload, needs_elevation, request.devo_home)
}

pub fn run_elevated_provisioning_setup(devo_home: &Path, real_user: &str) -> Result<()> {
    let sbx_dir = sandbox_dir(devo_home);
    std::fs::create_dir_all(&sbx_dir).map_err(|err| {
        failure(
            SetupErrorCode::OrchestratorSandboxDirCreateFailed,
    ...
    let payload = ElevationPayload {
        ...
        offline_username: OFFLINE_USERNAME.to_string(),
        online_username: ONLINE_USERNAME.to_string(),
        devo_home: devo_home.to_path_buf(),
        command_cwd: devo_home.to_path_buf(),
        read_roots: Vec::new(),
        write_roots: Vec::new(),
    ...
    run_setup_exe(&payload, /*needs_elevation*/ false, devo_home)
}
...
fn build_payload_roots(
    ...
    let mut read_roots = if let Some(roots) = overrides.read_roots.as_deref() {
        ...
        let mut read_roots = gather_helper_read_roots(request.devo_home);
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, devo_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to DEVO_HOME or anything under
    // DEVO_HOME/.sandbox, DEVO_HOME/.sandbox-bin, or DEVO_HOME/.sandbox-secrets.
    // These locations contain sandbox control/state and helper binaries and must
    // remain tamper-resistant.
    let devo_home_key = canonical_path_key(devo_home);
    let sbx_dir_key = canonical_path_key(&sandbox_dir(devo_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let sbx_bin_dir_key = canonical_path_key(&sandbox_bin_dir(devo_home));
    let sbx_bin_dir_prefix = format!("{}/", sbx_bin_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(devo_home));
...
mod tests {
    ...
    fn setup_completion_requires_ready_artifacts() {
        let devo_home = TempDir::new().expect("tempdir");
        let err = verify_setup_completed(devo_home.path())
    ...
    fn setup_exe_lookup_checks_package_resource_dir_for_bin_exe() {
        ...
        let exe = bin_dir.join("devo.exe");
        let setup_exe = resources_dir.join("devo-windows-sandbox-setup.exe");
        fs::write(&exe, b"devo").expect("write exe");
    ...
    #[test]
    fn expanded_write_roots_still_drop_protected_devo_home() {
        let tmp = TempDir::new().expect("tempdir");
        let user_profile = tmp.path().join("user-profile");
        let devo_home = user_profile.join("DevoHome");
        let documents = user_profile.join("Documents");
        ...
        let roots = super::filter_sensitive_write_roots(roots, &devo_home);
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        let extra_legacy_agent = extra_write_root.join(".codex");
        let explicit_deny = tmp.path().join("explicit-deny");
        fs::create_dir_all(&command_git).expect("create command .git");
        fs::create_dir_all(&extra_legacy_agent).expect("create extra .codex");
        let writable_roots = vec![
        ...
        assert_eq!(
            [
                ...
                dunce::canonicalize(&extra_legacy_agent).expect("canonical extra .codex"),
                explicit_deny,
            ]
            .into_iter()
