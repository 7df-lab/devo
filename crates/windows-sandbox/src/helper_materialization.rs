...
pub(crate) const RESOURCES_DIRNAME: &str = "devo-resources";
...
impl HelperExecutable {
    fn file_name(self) -> &'static str {
        match self {
            Self::CommandRunner => "devo-command-runner.exe",
        }
    }
...
pub(crate) fn helper_bin_dir(devo_home: &Path) -> PathBuf {
    sandbox_bin_dir(devo_home)
}
...
pub(crate) fn resolve_helper_for_launch(
    ...
    match copy_helper_if_needed(kind, devo_home, log_dir) {
        Ok(path) => {
            log_note(
                &format!(
...
pub fn resolve_current_exe_for_launch(devo_home: &Path, fallback_executable: &str) -> PathBuf {
    let source = match std::env::current_exe() {
        Ok(path) => path,
        Err(_) => return PathBuf::from(fallback_executable),
    };
    resolve_exe_for_launch(&source, devo_home)
}

pub fn resolve_exe_for_launch(source: &Path, devo_home: &Path) -> PathBuf {
    let Some(file_name) = source.file_name() else {
        return source.to_path_buf();
    };
    let destination = helper_bin_dir(devo_home).join(file_name);
    match copy_from_source_if_needed(source, &destination) {
        Ok(_) => destination,
        Err(err) => {
            let sandbox_log_dir = crate::sandbox_dir(devo_home);
            log_note(
                &format!(
...
pub(crate) fn copy_helper_if_needed(
    ...
    let cache_key = format!("{}|{}", kind.file_name(), devo_home.display());
    ...
    let destination = helper_destination_for_source(kind, devo_home, &source)?;
    log_note(
        &format!(
...
fn helper_destination_for_source(
    ...
    Ok(helper_bin_dir(devo_home).join(file_name))
}
...
mod tests {
    ...
    fn helper_bin_dir_is_under_sandbox_bin() {
        let devo_home = Path::new(r"C:\Users\example\.codex");

        assert_eq!(
            PathBuf::from(r"C:\Users\example\.codex\.sandbox-bin"),
            helper_bin_dir(devo_home)
        );
    }
    ...
    fn copy_runner_into_shared_bin_dir() {
        ...
        let runner_source = source_dir.join("devo-command-runner.exe");
        fs::write(&runner_source, b"runner").expect("runner");
        let runner_suffix = helper_version_suffix(&runner_source).expect("runner suffix");
        let runner_destination = helper_bin_dir(&devo_home).join(materialized_file_name(
    ...
    fn helper_source_lookup_checks_resource_dir() {
        ...
        let exe = release_dir.join("devo.exe");
        let helper = resources_dir.join("devo-command-runner.exe");
        fs::write(&exe, b"devo").expect("write exe");
        fs::write(&helper, b"runner").expect("write helper");

        let resolved =
            bundled_executable_path_for_exe(&exe, /*file_name*/ "devo-command-runner.exe")
                .expect("helper path");
    ...
    fn helper_source_lookup_prefers_package_resource_dir_over_bin_resource_dir() {
        ...
        let exe = bin_dir.join("devo.exe");
        let package_helper = package_resources_dir.join("devo-command-runner.exe");
        let bin_helper = bin_resources_dir.join("devo-command-runner.exe");
        fs::write(&exe, b"devo").expect("write exe");
    ...
    fn helper_source_lookup_prefers_direct_sibling_over_resource_dir() {
        ...
        fs::create_dir_all(&resources_dir).expect("create resources dir");
        let exe = release_dir.join("devo.exe");
        let sibling_helper = release_dir.join("devo-command-runner.exe");
        let resource_helper = resources_dir.join("devo-command-runner.exe");
        fs::write(&exe, b"devo").expect("write exe");
    ...
    fn materialized_file_name_adds_suffix_before_extension() {
        ...
        assert_eq!(file_name, "devo-command-runner-test-suffix.exe");
    }
}
