...
pub(super) fn ensure_devo_app_runtime_paths_readable(
...
fn runtime_paths(local_app_data: Option<PathBuf>, user_profile: Option<PathBuf>) -> Vec<PathBuf> {
    ...
    if let Some(local_app_data) = local_app_data {
        // Third-party OpenAI Codex desktop install (legacy interop).
        let third_party_openai_codex_root = local_app_data.join("OpenAI").join("Codex");
        runtime_paths.extend([
            third_party_openai_codex_root.join("bin"),
    ...
    if let Some(user_profile) = user_profile {
        runtime_paths.push(user_profile.join(".cache").join("devo-runtimes"));
    }
