...
use devo_windows_sandbox::dpapi_protect;
...
pub const SANDBOX_USERS_GROUP: &str = "DevoSandboxUsers";
const SANDBOX_USERS_GROUP_COMMENT: &str = "Devo sandbox internal group (managed)";
...
fn write_secrets(
    ...
    let secrets_dir = sandbox_secrets_dir(devo_home);
...
pub(super) fn prepare_setup_marker(devo_home: &Path, real_user: &str) -> Result<()> {
    let marker_path = sandbox_dir(devo_home).join("setup_marker.json");
