...
use devo_windows_sandbox::SetupFailure;
use devo_windows_sandbox::add_deny_write_ace;
use devo_windows_sandbox::canonicalize_path;
use devo_windows_sandbox::convert_string_sid_to_sid;
use devo_windows_sandbox::ensure_allow_mask_aces_with_inheritance;
use devo_windows_sandbox::ensure_allow_write_aces;
use devo_windows_sandbox::extract_setup_failure;
use devo_windows_sandbox::hide_newly_created_users;
use devo_windows_sandbox::install_wfp_filters;
use devo_windows_sandbox::is_command_cwd_root;
use devo_windows_sandbox::log_note;
use devo_windows_sandbox::log_writer;
use devo_windows_sandbox::path_mask_allows;
use devo_windows_sandbox::sandbox_bin_dir;
use devo_windows_sandbox::sandbox_dir;
use devo_windows_sandbox::sandbox_secrets_dir;
use devo_windows_sandbox::string_from_sid_bytes;
use devo_windows_sandbox::sync_persistent_deny_read_acls;
use devo_windows_sandbox::to_wide;
use devo_windows_sandbox::workspace_write_cap_sid_for_root;
use devo_windows_sandbox::workspace_write_root_overlaps_path;
use devo_windows_sandbox::write_setup_error_report;
...
fn devo_home_from_env() -> Option<String> {
    std::env::var("DEVO_HOME")
...
pub fn main() -> Result<()> {
    ...
    if let Err(e) = &ret {
        ...
        if let Some(devo_home) = devo_home_from_env() {
            let sbx_dir = sandbox_dir(Path::new(&devo_home));
...
fn real_main() -> Result<()> {
    ...
    let sbx_dir = sandbox_dir(&payload.devo_home);
    ...
    if let Err(err) = &result {
        ...
        if let Err(write_err) = write_setup_error_report(&payload.devo_home, &report) {
            let _ = log_line(
                &mut log,
...
fn run_setup(payload: &Payload, log: &mut dyn Write, sbx_dir: &Path) -> Result<()> {
    ...
    if writes_setup_marker {
        prepare_setup_marker(&payload.devo_home, &payload.real_user)?;
    }
    match payload.mode {
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_dir(&payload.devo_home).display()
            ),
        ))
    })?;
    lock_sandbox_dir(
        &sandbox_secrets_dir(&payload.devo_home),
        &payload.real_user,
        sandbox_group_sid,
        DENY_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_secrets_dir(&payload.devo_home).display()
            ),
        ))
    })?;
    let legacy_users = sandbox_dir(&payload.devo_home).join("sandbox_users.json");
...
fn lock_sandbox_bin_dir(
    ...
    lock_sandbox_dir(
        &sandbox_bin_dir(&payload.devo_home),
        &payload.real_user,
        sandbox_group_sid,
        GRANT_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_EXECUTE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_bin_dir(&payload.devo_home).display()
            ),
        ))
    })
}
...
fn run_setup_full(payload: &Payload, log: &mut dyn Write, sbx_dir: &Path) -> Result<()> {
    ...
    if refresh_only {
        setup_runtime_bin::ensure_devo_app_runtime_paths_readable(
            sandbox_group_psid,
            &mut refresh_errors,
            log,
        )?;
    ...
    for root in &payload.write_roots {
        ...
        let root_cap_sid_str =
            workspace_write_cap_sid_for_root(&payload.devo_home, &payload.command_cwd, root)?;
...
mod tests {
    ...
    fn payload_json() -> serde_json::Value {
        json!({
            ...
            "offline_username": "DevoSandboxOffline",
            "online_username": "DevoSandboxOnline",
            "devo_home": "C:\\devo-home",
    ...
    fn write_root_refresh_replaces_stale_delete_child_grant() {
        ...
        let sid = workspace_write_cap_sid_for_root(&devo_home, &workspace, &workspace)
    ...
    fn deny_path_under_active_root_uses_only_matching_root_sid() {
        ...
        let stale_sid = workspace_write_cap_sid_for_root(&devo_home, &workspace, &stale_root)
            .expect("stale sid");
        let active_sid = workspace_write_cap_sid_for_root(&devo_home, &workspace, &active_root)
            .expect("active sid");
    ...
    fn deny_path_includes_nested_active_root_sid() {
        ...
        let workspace_sid = workspace_write_cap_sid_for_root(&devo_home, &workspace, &workspace)
            .expect("workspace sid");
        let nested_sid = workspace_write_cap_sid_for_root(&devo_home, &workspace, &nested_root)
            .expect("nested sid");
