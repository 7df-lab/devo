...
use anyhow::Context;
use anyhow::Result;
use devo_windows_sandbox::ConsoleMode;
use devo_windows_sandbox::ErrorPayload;
use devo_windows_sandbox::ErrorStage;
use devo_windows_sandbox::ExitPayload;
use devo_windows_sandbox::FramedMessage;
use devo_windows_sandbox::IPC_PROTOCOL_VERSION;
use devo_windows_sandbox::LocalSid;
use devo_windows_sandbox::Message;
use devo_windows_sandbox::OutputPayload;
use devo_windows_sandbox::OutputStream;
use devo_windows_sandbox::PipeSpawnHandles;
use devo_windows_sandbox::ResizePayload;
use devo_windows_sandbox::SpawnReady;
use devo_windows_sandbox::SpawnRequest;
use devo_windows_sandbox::StderrMode;
use devo_windows_sandbox::StdinMode;
use devo_windows_sandbox::WindowsSandboxTokenMode;
use devo_windows_sandbox::allow_null_device;
use devo_windows_sandbox::create_readonly_token_with_caps_and_user_from;
use devo_windows_sandbox::create_workspace_write_token_with_caps_and_user_from;
use devo_windows_sandbox::decode_bytes;
use devo_windows_sandbox::encode_bytes;
use devo_windows_sandbox::get_current_token_for_restriction;
use devo_windows_sandbox::hide_current_user_profile_dir;
use devo_windows_sandbox::log_note;
use devo_windows_sandbox::read_frame;
use devo_windows_sandbox::read_handle_loop;
use devo_windows_sandbox::spawn_process_with_pipes;
use devo_windows_sandbox::to_wide;
use devo_windows_sandbox::token_mode_for_permission_profile;
use devo_windows_sandbox::write_frame;
use std::ffi::OsStr;
use std::fs::File;
...
// Kept in sync with the main Devo binary fs-helper flag without introducing
// a dependency on the CLI crate from this helper executable.
const FS_HELPER_ARG: &str = "--devo-run-as-fs-helper";
const READ_ACL_MUTEX_NAME: &str = "Local\\DevoSandboxReadAcl";
...
struct IpcSpawnedProcess {
    ...
    conpty_owner: Option<devo_windows_sandbox::ConptyInstance>,
...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    let log_dir = req.devo_home.clone();
    hide_current_user_profile_dir(req.devo_home.as_path());
    ...
    let (pi, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, mut conpty) = devo_windows_sandbox::spawn_conpty_process_as_user(
            h_token.raw(),
            &req.command,
            &effective_cwd,
            &req.env,
