...
use crate::sandbox_utils::ensure_devo_home_exists;
...
use crate::workspace_acl::protect_workspace_legacy_agent_dir;
use anyhow::Context;
use anyhow::Result;
...
fn prepare_spawn_context_common(
    ...
    let sandbox_base = devo_home.join(".sandbox");
...
pub(crate) fn root_capability_sids(
    ...
    for root in roots {
        let sid_str = workspace_write_cap_sid_for_root(devo_home, cwd, &root)?;
    ...
    unsafe {
                ...
                matching_root_capability(current_dir, acl_sids.write_root_sids)
        {
            ...
            if is_command_cwd_root(&workspace_sid.root, &canonical_cwd) {
                let _ = protect_workspace_legacy_agent_dir(current_dir, workspace_sid.sid.as_ptr());
...
pub(crate) fn prepare_elevated_spawn_context_for_permissions(
    ...
    let sandbox_base = devo_home.join(".sandbox");
    ensure_devo_home_exists(&sandbox_base)?;
    ...
    let (psid_to_use, cap_sids) = if uses_write_capabilities {
        let cap_sids = root_capability_sids(devo_home, cwd, effective_write_roots)?
            .into_iter()
...
mod tests {
    ...
    fn root_capability_sids_only_include_active_roots() {
        ...
        let sid_strs =
            root_capability_sids(&devo_home, &workspace, vec![workspace.clone(), active_root])
    ...
    fn legacy_deny_path_includes_nested_active_root_sid() {
        ...
        let nested_sid = workspace_write_cap_sid_for_root(&devo_home, &workspace, &nested_root)
            .expect("nested sid");
        let unrelated_sid =
            workspace_write_cap_sid_for_root(&devo_home, &workspace, &unrelated_root)
    ...
    fn legacy_capability_roots_use_effective_write_roots() {
        ...
        let workspace = temp.path().join("workspace");
        let active_root = temp.path().join("active-root");
        let sandbox_root = devo_home.join(".sandbox");
        std::fs::create_dir_all(&devo_home).expect("create devo home");
        std::fs::create_dir_all(&workspace).expect("create workspace");
        std::fs::create_dir_all(&active_root).expect("create active root");
        ...
        let writable_roots = vec![
            ...
            AbsolutePathBuf::try_from(devo_home.as_path()).expect("devo home"),
        ...
        let roots =
            legacy_session_capability_roots(&permissions, &workspace, &HashMap::new(), &devo_home);
        ...
        assert!(!roots.contains(&dunce::canonicalize(&devo_home).expect("devo home")));
