...
use super::DEVO_HOME_FLAG;
use super::DEVO_WINDOWS_SANDBOX_ARG1;
...
fn windows_wrapper_args_round_trip() {
    ...
    let args = create_windows_sandbox_command_args_for_permission_profile(
        vec![
            "devo.exe".to_string(),
            "--devo-run-as-fs-helper".to_string(),
        ],
        &command_cwd,
    ...
    assert!(args.contains(&DEVO_HOME_FLAG.to_string()));
    ...
    assert_eq!(parsed.command, vec!["devo.exe", "--devo-run-as-fs-helper"]);
