...
pub fn current_log_file_path_for_devo_home(devo_home: &Path) -> PathBuf {
    current_log_file_path(&crate::sandbox_dir(devo_home))
}
...
mod tests {
    ...
    #[test]
    fn current_log_file_path_for_devo_home_uses_sandbox_dir() {
        let devo_home = Path::new("devo-home");

        assert_eq!(
            current_log_file_path_for_devo_home(devo_home),
            current_log_file_path(&devo_home.join(".sandbox"))
        );
    }
}
