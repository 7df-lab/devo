...
# Run a suite of smoke tests against the Windows sandbox via the Devo CLI
...
def _resolve_devo_cmd() -> List[str]:
    """Resolve the Devo CLI to invoke `devo sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run Devo.
    """
    ...
    candidates = [
        ws_root / "target" / "debug" / "devo.exe",
        ws_root / "target" / "release" / "devo.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend([
            cargo_base / "debug" / "devo.exe",
            cargo_base / "release" / "devo.exe",
        ])
    ...
    if shutil.which("devo"):
        return ["devo"]

    raise FileNotFoundError(
        "Devo CLI not found. Build it first, e.g.\n"
        "  cargo build -p devo-cli --release\n"
        "or for debug:\n"
        "  cargo build -p devo-cli\n"
    )

DEVO_CMD = _resolve_devo_cmd()
print(DEVO_CMD)
TIMEOUT_SEC = 20
...
def run_sbx(
    ...
    # Map policy to Devo CLI overrides.
    ...
    argv = [*DEVO_CMD, "sandbox", "windows", *policy_flags, *overrides, "--", *cmd_argv]
    print(cmd_argv)
...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            ...
            rc_direct, _out_direct, err_direct = run_sbx(
                ...
                direct_cmd,
                WS_ROOT,
                env_extra={"DEVO_HOME": str(proxy_home)},
            )
    ...
    rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo hi > \\\\.\\pipe\\devo_testpipe"], WS_ROOT)
    ...
    devo_home = Path(os.environ["USERPROFILE"]) / ".codex"
    cap_sid_target = devo_home / "cap_sid"
    rc, out, err = run_sbx(
        "workspace-write",
    ...
    if make_symlink(fake_root, Path("C:/")):
        rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo owned > devo_escape.txt"], fake_root)
    ...
    rc, out, err = run_sbx(
        ...
        [
            ...
            "-NoProfile",
            "-Command",
            "Start-Process 'https://devo-invalid.local/smoke'",
        ],
        WS_ROOT,
    )
    add(
